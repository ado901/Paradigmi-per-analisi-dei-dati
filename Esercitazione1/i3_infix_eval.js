...
class Actions {
    ...
    //Restituisce -x, es. dato 4 restituisce -4
    opp(x) {
        return -x;
    }
    //Restituisce il valore numerico di x come float, es. dato '4' restituisce 4.0
    num(x) {
        return parseFloat(x);
    }
    //Restituisce il valore della variabile x, cercandolo come key in this.values, es. se x='w', restituisce il valore di this.values['w']
    var(x) {
...
class Tokenizer {
    ...
    /*
    La funzione peek cerca e restituisce il successivo token in text (null se non trova nulla), a partire dal
...
// Variabili che possiamo utilizzare nelle espressioni che andremo a valutare, e il loro valore (non modificabile).
let values = {'w': 0.0, 'x': 1.0, 'y': 1.5, 'z': 0.5};
let act = new Actions(values);
//Espressioni per testare, nella forma espressione:risultato_atteso.
let expressions = {'(((1.5)))': 1.5, 'w * -z': 0, 'x / z * -y': -3, 'x / 0.5 * --y': 3, 'w': 0, '(x + w) * (x + y) * (y - z)': 2.5};
